...
int main(int argc, char* argv[])
{
  ...
  // double initial_temp = 2*pow(10,initial_exp);
  ...
  double free_energy = find_free_energy(initial_Hamiltonian, final_temp, randsigma, placeholder);
    ...
    for(int i=10; i>=2; i--)
    {
      ...
      for(int sweep=0; sweep<no_sweeps; sweep++)
      {
        for(int lattice_index=0; lattice_index<size; lattice_index++)
        {
          ...
          suggested_free_energy = find_free_energy(suggested_Hamiltonian, temperature, suggested_randsigma, placeholder);
      ...
      cout << "temperature = " << temperature << " done. \r";
