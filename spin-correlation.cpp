...
double spin_correlation(int r, MatrixXd& randsigma)
{
  double s0_sr = 0;
  VectorXd s0,sr;
  s0 = sr = VectorXd::Zero(3);
  for(int i=0; i<size; i++)
  {
    s0_sr += (randsigma.row(i)).dot(randsigma.row((i+r)%size));
    sr += randsigma.row((i+r)%size);
    s0 += randsigma.row(i);
  }
  s0_sr /= size; sr /= size; s0 /= size;
  double corr = s0_sr - s0.dot(sr);
  return corr;
}
...
int main(int argc, char* argv[])
{
  ...
  filename="data/spin_correlation"+current_time_str()+latticedata+".txt"; ofstream outfile_corr(filename);
  ...
  for(int i=0; i<size; i++)
  {
    outfile_corr << i << " " << spin_correlation(i,randsigma) << endl;
  }
